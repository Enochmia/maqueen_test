...
basic.pause(2000) //조금의 시간이 지난 다음 학습해야 좀 더 확실하게 학습이 됨
...
basic.forever(function () { //함수 지정 - JS는 일케 함수 지정하나봐요 JS는 저도 거의 첨이라
    ...
    dis = maqueen.Ultrasonic(PingUnit.Centimeters)
    ...
    if (huskylens.isAppear(1, HUSKYLENSResultType_t.HUSKYLENSResultBlock) == true) { //아까 학습했던 "ID 1"이 화면에 나와있는지
        ...
        maqueen.motorRun(maqueen.Motors.All, maqueen.Dir.CW, 100) //기본 속도 설정
